    ...
    try {
      ...
      try {
        ...
        const odooMembers = Array.from(uniqueOdooMap.values()).map(row => ({
          ...
          isImported: true,
          phone: '',
          // 📦 Rich archive details — used by the panel and the date filters
          plan: row.membership_name || 'Archive',
          subscription_name: row.membership_name || null,
          subscriptionName: row.membership_name || null,
          created_at: row.date_inscription || null,
          createdAt: row.date_inscription || null,
          period_from: row.date_from || row.date_inscription || null,
          periodFrom: row.date_from || row.date_inscription || null,
          periodTo: row.expires_on || null,
          total_paid: Number(row.amount_paid) || 0,
          totalPaid: Number(row.amount_paid) || 0,
          contract_number: row.pos_order || null,
          contractNumber: row.pos_order || null,
          odooState: row.status || null,
          odooPartnerId: row.partner_id || null,
    ...
    try {
      ...
      if (!member && memberId.startsWith('odoo-')) {
        ...
        if (row) {
          member = {
            ...
            full_name: row.full_name,
            phone: '',
            // 📦 Rich Odoo archive details (formule, montant, dates, POS order)
            plan: row.membership_name || 'Archive',
            ...
            totalPaid: Number(row.amount_paid) || 0,
            payments: null,
            createdAtStr: row.date_inscription || row.expires_on,
            createdAt: row.date_inscription || null,
            source: 'odoo',
            isArchive: true,
            is_archive: 1,
      ...
      if (member) {
        ...
        member = {
          ...
          createdAtStr:    member.createdAtStr    || member.createdAt        || member.created_at || null,
          // Carry archive flags + the rich Odoo block through the normalization
